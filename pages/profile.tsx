...
export default function Profile({ userDetails }: ProfileProps) {
  ...
  const [files, setFiles] = useState<any[]>([userDetails?.avatar_url ?? ''])
  ...
  const handleUserDetailsSave = async (evt) => {
    ...
    const { error: saveError } = await supabase.from('user_details')
      .update({
        full_name: fullName,
    ...
    setLoading(false)
    if (saveError) {
      setMessage({ type: 'error', content: 'An error occurred while saving your details.' })
    } else {
    ...
    <section className="mb-32">
      <div className="max-w-6xl mx-auto pt-8 sm:pt-24 pb-8 px-4 sm:px-6 lg:px-8">
        <div className="sm:flex sm:flex-col sm:align-center">
          ...
          <p className="mt-4 text-xl text-center sm:text-2xl max-w-2xl m-auto">
            Update your details
          </p>
        </div>
      </div>
      ...
      <div className="p-4">
        <div className="border border-secondary max-w-3xl w-full rounded-md m-auto my-4">
          <div className="px-5 py-4">
            <form onSubmit={handleUserDetailsSave} className="flex flex-col items-center space-y-4">
            ...
            {
              message.content &&
              <div className={`${message.type === 'error' ? 'text-red-300' : 'text-gray-400'} text-center border 'border-gray-400' border-solid mt-4 p-3`}>
                {message.content}
              </div>
            }
          </div>
